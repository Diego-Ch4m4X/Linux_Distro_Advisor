...
[![Edição: 0.1.7](https://img.shields.io/badge/edi%C3%A7%C3%A3o-0.1.7-8250df)](#historico-desta-edicao)
...
Este guia fornece o vocabulário e o método. O QUIZ [Linux Distro Advisor](https://diego-ch4m4x.github.io/Linux_Distro_Advisor/quiz.html) aplica uma **heurística documentada e versionada** sobre o mesmo modelo e devolve uma recomendação principal, duas alternativas e os trade-offs relevantes.
...
flowchart LR
    ...
    C --> D
    D --> E["Obtenha shortlist e trade-offs"]
...
| **Compatibilidade** | Que contratos técnicos precisam ser preservados? | API (*Application Programming Interface*), ABI (*Application Binary Interface*), kABI (*kernel Application Binary Interface*), formatos de configuração |
...
└── ferramentas: APT (*Advanced Package Tool*) sobre pacotes deb/dpkg
~~~
...
| **EndeavourOS** | Rolling Release | acompanha de perto repositórios Arch | pacman + **AUR (*Arch User Repository*)** opcional | Snapshots/ISOs não criam releases fixas. |
...
| **SONiC** | Varia por build/vendor | community e vendors publicam trains próprios | imagem de switch **NOS (*Network Operating System*)** | O ciclo depende da distribuição SONiC e do fornecedor. Sempre valide train de release, **ASIC (*Application-Specific Integrated Circuit*)** e hardware específicos. |
...
```mermaid
flowchart TD
    A["sources.list acompanha testing"] --> A1["Forky é Testing"]
    A1 --> A2["Forky vira Stable"]
    A2 --> A3["testing passa a apontar para a próxima Testing"]

    B["sources.list acompanha forky"] --> B1["Forky é Testing"]
    B1 --> B2["Forky vira Stable"]
    B2 --> B3["O host continua acompanhando Forky"]
```
...
```mermaid
flowchart TD
    A["Deployment A — Fedora 44 em execução"] -->|"prepara novo deployment"| B["Deployment B — Fedora 44 atualizado"]
    B -->|"reboot"| C["Deployment B em execução"]
    C -->|"rollback, se o deployment anterior estiver disponível"| A
    V["/var permanece persistente e fora do deployment"] --- A
    V --- B
```
...
No Ubuntu, por exemplo, o escopo da manutenção padrão, o conteúdo de Main, a cobertura adicional por Ubuntu Pro/**ESM (*Expanded Security Maintenance*)** e os pacotes de Universe precisam ser distinguidos. Em distribuições empresariais, add-ons e fases estendidas também possuem condições próprias.
...
- estratégias como **3-2-1-1-0** — por exemplo, múltiplas cópias em mídias/localizações distintas, uma cópia offline/imutável e verificação de ausência de erros — podem ser usadas como referência de desenho de backup, mas não são garantia universal: a política deve refletir **RPO (*Recovery Point Objective*, perda máxima de dados tolerada)**, **RTO (*Recovery Time Objective*, tempo-alvo máximo de recuperação)**, ameaças, retenção, cópias imutáveis/offline e testes de recuperação.
...
| **Contrato formal** | certificação, SLA ou matriz **ISV (*Independent Software Vendor*)** | obrigação/escopo formal conforme documento |
...
| **Publicação/revisão da edição 0.1.7** | **2026-08-22** | data da revisão editorial/técnica desta edição |
| **Última validação estrutural interna** | **2026-08-22** | verificação de âncoras, links internos, fences, tabelas, Mermaid e invariantes documentais |
| **Última revisão semântica desta edição** | **2026-08-22** | conferência de coerência entre seção, tabela, glossário, primeiras ocorrências e finalidade dos itens alterados |
...
A edição **0.1.7** mantém **95 candidatos**, não “95 distribuições puras”. O conjunto inclui distribuições, edições, canais, sistemas image-based, network operating systems e appliances Linux.
...
flowchart TD
    ...
    E --> F["Ranking relativo + motivos + trade-offs"]
...
├── quiz.html      # questionário interativo standalone
├── sitemap.xml    # URLs canônicas públicas para descoberta/indexação
└── LICENSE
~~~

O **README.md** permanece a fonte canônica dos conceitos, critérios e limites e usa diagramas Mermaid renderizados nativamente pelo GitHub quando a visualização reduz carga cognitiva. O **index.html** é a camada pública de descoberta e leitura para GitHub Pages/SEO, derivada editorialmente do README. O **quiz.html** implementa o questionário e carrega o catálogo factual detalhado. Mudanças que alterem conceitos ou decisões devem preservar a coerência entre essas três superfícies.

<a id="22-publicacao"></a>

### Publicação

O **index.html** é o ponto de entrada do GitHub Pages: contém texto HTML semântico e indexável, metadados de SEO, links para o guia e para o quiz, e diagramas Mermaid que complementam — sem substituir — o conteúdo textual. O **quiz.html** continua sendo a aplicação interativa. O README é lido diretamente no repositório e continua sendo a referência editorial canônica.

#### SEO técnico do GitHub Pages

A publicação web mantém regras simples para reduzir ambiguidade e divergência entre superfícies:
...
- `sitemap.xml` lista somente essas URLs públicas canônicas; o `README.md`, hospedado em `github.com`, não integra o sitemap do GitHub Pages;
- `<lastmod>` deve refletir apenas atualização significativa do conteúdo, links ou dados estruturados da respectiva página;
- `priority` e `changefreq` não são usados no sitemap, pois não agregam sinal útil ao Google;
- um `robots.txt` válido para rastreadores precisa estar na raiz do **hostname**. Em um Project Site do GitHub Pages, `.../Linux_Distro_Advisor/robots.txt` não equivale a `https://diego-ch4m4x.github.io/robots.txt`; por isso este repositório não depende de um `robots.txt` local para permitir indexação;
- após publicar, a propriedade por **prefixo de URL** pode ser verificada no Google Search Console e o `sitemap.xml` deve ser enviado por lá. Canonical e sitemap são sinais de descoberta/preferência, não garantia de indexação ou posicionamento.

#### Política de Mermaid no README

O GitHub renderiza blocos `mermaid` diretamente em arquivos Markdown. Como a versão efetiva do renderer é controlada pelo próprio GitHub e pode mudar, este README adota uma política conservadora:

- usar principalmente `flowchart`, `subgraph`, rótulos e arestas — recursos maduros e amplamente suportados;
- não fixar tema, cores ou CSS no diagrama, permitindo que o GitHub preserve contraste nos modos claro e escuro;
- evitar sintaxes experimentais/beta e recursos cuja legibilidade dependa de JavaScript customizado;
- manter toda informação essencial também no texto, tabela ou lista ao redor do diagrama;
- validar a renderização no GitHub antes de publicar uma tag. A própria documentação do GitHub recomenda o diagrama `info` quando for necessário verificar a versão Mermaid em uso.

Os diagramas do **index.html**, por sua vez, usam Mermaid carregado por CDN. Se essa dependência externa estiver indisponível, o conteúdo textual, a navegação e os links do site continuam acessíveis; apenas a renderização gráfica pode não ocorrer. Essa dependência pertence ao `index.html` e **não altera a propriedade standalone do `quiz.html`**.

Checklist de release:

> **Invariantes recomendados para automação/CI:** o número de candidatos documentados deve coincidir com o catálogo usado pelo quiz; cada candidato deve possuir identificador único; classificações compartilhadas entre README e quiz não devem divergir; links/âncoras e blocos Mermaid precisam ser válidos; mudanças factuais devem registrar fonte e data.

1. atualizar snapshot e fontes primárias quando a edição alterar dados factuais;
2. executar **auditoria estrutural**: âncoras, links internos, fences, tabelas, blocos Mermaid e invariantes de contagem/schema;
...
5. testar teclado, mobile, impressão e tema nos HTMLs e validar os diagramas Mermaid nos modos claro/escuro do GitHub;
6. revisar links internos/externos, `canonical`, metadados indexáveis e coerência entre `index.html`, `quiz.html` e `sitemap.xml`;
7. atualizar `<lastmod>` do sitemap somente quando houver mudança significativa na página correspondente;
8. registrar mudanças de pesos, candidatos e fatos voláteis;
9. criar tag da edição.
...
- simplificados os dados estruturados do `index.html` para `WebSite` + `WebPage`, evitando marcação de `SoftwareApplication` incompleta para os requisitos de rich result do Google;
...
- mantido o snapshot factual de **2026-08-15** e preservado o contrato funcional do quiz;
- reorganizada a publicação web sem alterar a heurística: o antigo `index.html` do quiz foi renomeado para `quiz.html`, e um novo `index.html` passou a funcionar como página pública/indexável derivada do README, com links corrigidos e diagramas Mermaid por CDN.
